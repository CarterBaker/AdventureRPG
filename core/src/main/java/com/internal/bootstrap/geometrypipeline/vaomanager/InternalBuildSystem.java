...
class InternalBuildSystem extends SystemPackage {
    ...
    VAOHandle addVAO(File file, InternalLoadManager loadManager) {
        ...
        if (vaoElement.isJsonArray())
            return createLayout(vaoElement.getAsJsonArray(), file);
    ...
    private VAOHandle getOrCreateVAO(String vaoName, InternalLoadManager loadManager, File currentFile) {
        ...
        if (existing == null)
            throwException("Failed to create VAO layout '" + vaoName + "' from file: " + referencedFile.getName());
    ...
    private VAOHandle createLayout(JsonArray jsonArray, File file) {
