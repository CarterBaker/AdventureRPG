...
import com.internal.platform.files.FileHandle;
...
public class Pixmap {
    ...
    public Pixmap(FileHandle handle) {
        try {
            BufferedImage image = ImageIO.read(handle.file());
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    ...
                    int b = c & 0xFF;
                    int idx = (y * width + x) * 4;
                    pixels.put(idx, (byte) r); pixels.put(idx + 1, (byte) g); pixels.put(idx + 2, (byte) b); pixels.put(idx + 3, (byte) a);
                }
            }
        } catch (IOException e) {
            throw new RuntimeException("Failed loading image: " + handle.path(), e);
        }
    }
