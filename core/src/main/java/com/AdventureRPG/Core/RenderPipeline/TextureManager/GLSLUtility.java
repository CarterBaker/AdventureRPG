...
class GLSLUtility {
    ...
    static int pushTextureArray(BufferedImage[] layers) {
        ...
        if (depth == 0)
            throw new GraphicException.TextureAliasException("There are no defined layers in the texture array");
        ...
        for (int i = 1; i < depth; i++) {
            ...
            if (b.getWidth() != width || b.getHeight() != height)
                throw new GraphicException.ImageSizeException("All images must be the same width and height");
        }

        // ensure GL30 available
        if (!(Gdx.gl instanceof GL30))
            throw new GraphicException.OpenGLException("GL30 required for texture arrays.");
        ...
        if (handle == 0) // TODO: Add my own error
            throw new GraphicException.OpenGLException("The GPU handle for the image could not be found");
