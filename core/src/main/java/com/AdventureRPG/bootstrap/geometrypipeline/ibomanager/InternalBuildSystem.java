package com.AdventureRPG.bootstrap.geometrypipeline.ibomanager;
...
class InternalBuildSystem extends SystemPackage {
    ...
    public IBOHandle addIBO(
        ...
        if (iboElement.isJsonArray())
            return createIBOFromData(
                    vaoHandle,
                    iboElement.getAsJsonArray(),
                    file);
    ...
    private JsonElement validateIBOElement(
            JsonObject json,
            File file) {
    ...
    private VAOHandle getOrCreateVAO(
            String vaoName,
            InternalLoadManager loadManager,
            File currentFile) {
    ...
    private IBOHandle createIBOFromData(
            VAOHandle vaoHandle,
