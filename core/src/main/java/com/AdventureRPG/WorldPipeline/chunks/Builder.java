...
public class Builder {
    ...
    private FloatArrayList tempQuadBuffer;
    ...
    public Builder(WorldPipeline worldPipeline, Chunk chunk) {
    ...
    public boolean build(int subChunkIndex) {
        ...
        try {
            ...
            if (quads.size > 0)
                buildFromQuads(subChunk.subChunkMeshData);
    ...
    private void buildFromQuads(SubChunkMeshData subChunkMeshData) {
        ...
        for (int i = 0; i < quads.size; i += QUAD_SIZE) {
            ...
            // Clear and reuse temp buffer
            tempQuadBuffer.clear();

            // Pack vertices into reusable buffer
            for (int vi = 0; vi < 4; vi++) {
                tempQuadBuffer.add(verts[vi][0]);
                tempQuadBuffer.add(verts[vi][1]);
                tempQuadBuffer.add(verts[vi][2]);
                tempQuadBuffer.add(direction.x);
                tempQuadBuffer.add(direction.y);
                tempQuadBuffer.add(direction.z);
                tempQuadBuffer.add(vi == 0 ? c0 : vi == 1 ? c1 : vi == 2 ? c2 : c3);
                tempQuadBuffer.add(uvs[vi][0]);
                tempQuadBuffer.add(uvs[vi][1]);
            }

            // Pass the FloatArrayList directly
            subChunkMeshData.addVertices(matId, tempQuadBuffer);
        }
    }
