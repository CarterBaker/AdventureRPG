...
public class EntityRenderSystem extends SystemPackage {
     ...
     * to RenderManager.pushSkinnedCall(). The mesh is scaled by
     * entity.getModelScale() — the ratio between this entity's own
     * gameplay size and its character mesh's authored dimensions — so
     * every character, player or NPC, renders at its actual gameplay
     * size regardless of what scale its mesh happens to be modeled at,
     * never at a fixed 1:1 mesh scale. Position is centered on the
     * entity's own footprint (worldPosition + size.x/2, size.z/2 on the
     * horizontal plane) using entity.getSize() directly — that's the
     * actual collision footprint, not a rendering concern — the exact
     * same centering PlayerManager already uses to place the camera's
     * eye position, so the rendered body and the point the camera
     * orbits/aims from always agree; worldPosition itself is the
    ...
    public void pushCharacter(
        ...
        RigMathUtility.composeLocal(
                positionScratch, rotationScratch, entity.getModelScale(),
