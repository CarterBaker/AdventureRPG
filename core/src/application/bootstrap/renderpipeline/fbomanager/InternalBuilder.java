...
class InternalBuilder extends BuilderPackage {
    ...
    FboInstance buildInstance(FboData data) {
        ...
        for (int i = 0; i < attachments.size(); i++) {
            ...
            int tex = GLSLUtility.genTexture();
            GLSLUtility.bindTexture(tex);

            if (attachment.isDepth()) {
                GLSLUtility.texImage2DDepth(width, height);
                GLSLUtility.texParameterNearest();
                GLSLUtility.framebufferTexture2DDepth(tex);
                depthTextures.add(tex);
            } else {
                GLSLUtility.texImage2DColor(attachment.getInternalFormat(), width, height);
                GLSLUtility.texParameterLinear();
                GLSLUtility.framebufferTexture2DColor(tex, colorIndex);
                textures.add(tex);
                colorIndex++;
            }
        ...
        if (colorIndex > 0)
            GLSLUtility.drawBuffers(colorIndex);

        if (GLSLUtility.checkFramebufferStatus() != EngineSetting.GL_FRAMEBUFFER_COMPLETE)
