...
public class AnimationStateHandle extends HandlePackage {
    ...
    /*
     * Per-entity runtime playback of an animation tree. Holds the tree, the
     * parameter values fed to it, every layer's current node, cross-fade
     * and phase, and the fully evaluated per-bone skinning matrices for the
     * current frame — the single output consumed by rendering. No manager
     * owns this — it lives directly on EntityInstance, same as
     * EntityStateHandle and StatisticsHandle.
     *
     * Each layer maps the entity's movement state to one of its nodes, and
     * a node blends its clips along a parameter with one shared normalized
     * phase. Switching nodes cross-fades with a smooth step while the
     * outgoing node keeps playing; a switch that interrupts a cross-fade
     * freezes the layer's blended pose and fades out of that instead, so
     * nothing ever snaps. A state change waits out the target node's delay
     * before it counts, and a layer whose state has no node fades out as a
     * whole. Layers stack in order through their bone masks — OVERRIDE
     * layers cross-fade the pose toward their own, ADDITIVE layers add
     * their keyframe offsets on top.
     *
     * Every pose value here is an offset from the rig's bind pose, exactly
     * as keyframes are authored, and the bind pose is added once per bone
     * before composition. Tracks are sampled with a cubic Hermite curve
     * whose tangents come from the neighbouring keyframes — wrapping around
     * the loop for a looping clip, flat at the ends of a one-shot.
     *
     * skinningMatrices[boneIndex] = currentWorld * S(proportion) *
     * rig.getBindWorldInverseMatrix(boneIndex). boneProportions is a
     * per-bone, non-inherited geometry scale layered on top of the tree, so
     * scaling one bone reshapes only the skin it owns; a child's joint
     * offset IS scaled by its parent's proportion. AppearanceHandle is the
     * only writer; every proportion is 1.0 for an entity with no appearance.
     *
