...
public class MenuRenderSystem extends SystemPackage {
    ...
    /*
     * Renders every visible menu element tree each frame. Resolves each
     * element's active state, sprite and color, including hover, pointed and
     * themed colors, fits label text to its element, and poses animated
     * elements from the menu's clock before drawing.
     */
    ...
    private void pushSpriteRenderCall(ElementInstance element, SpriteInstance sprite, MenuColorStruct color) {
        ...
        sprite.getModelInstance().getMaterial().setUniform(EngineSetting.UNIFORM_TRANSFORM, element.getTransform());
