...
class ShaderBuilder extends BuilderPackage {
    ...
    // Internal
    private ShaderLoader internalLoader;
    private File root;
    ...
    @Override
    protected void get() {
        this.internalLoader = get(ShaderLoader.class);
    ...
    void parseShaderFile(ShaderSourceStruct source) {
        ...
        String rawText = ShaderParserUtility.convertFileToRawText(source.getShaderFile());
        ObjectArrayList<String> lines = ShaderParserUtility.convertRawTextToArray(rawText);
    ...
    private String parseVersionInfo(ObjectArrayList<String> lines) {
        for (int i = 0; i < lines.size(); i++)
            if (ShaderParserUtility.lineStartsWith(lines.get(i), "#version"))
                return lines.get(i);
        return null;
    }
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            if (isUniformBlockStart(line)) {
                ...
                if (currentBlockName != null) {
                    insideBlock = true;
                    braceDepth = ShaderParserUtility.countCharInString(line, '{') -
                            ShaderParserUtility.countCharInString(line, '}');
                }
    ...
    private void parseUniformDeclaration(String declaration, ShaderSourceStruct source) {
        ...
        int lastDelimiter = ShaderParserUtility.findLastTypeDelimiter(declaration);
        ...
        for (int i = 0; i < names.length; i++) {
            ...
            if (name.contains("[")) {
                ...
                if (bracketStart != -1 && bracketEnd > bracketStart) {
                    ...
                    arrayCount = ShaderParserUtility.parseIntOrDefault(countStr, 0);
    ...
    private void parseIncludes(ShaderSourceStruct source, ObjectArrayList<String> lines) {
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            if (ShaderParserUtility.lineStartsWith(line, "#include")) {

                String includePath = ShaderParserUtility.extractPayloadAfterToken(line, "#include");
