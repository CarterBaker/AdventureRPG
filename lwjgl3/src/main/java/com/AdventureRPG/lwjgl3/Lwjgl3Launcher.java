...
public class Lwjgl3Launcher {
    ...
    private static void saveWindowInfoOnClose(File file, Settings settings) {
        ...
        if (PlatformRuntime.graphics instanceof Lwjgl3Graphics) {
            Lwjgl3Window window = ((Lwjgl3Graphics) PlatformRuntime.graphics).getWindow();
            settings.windowWidth = PlatformRuntime.graphics.getWidth();
            settings.windowHeight = PlatformRuntime.graphics.getHeight();
            settings.windowX = window.getPositionX();
            settings.windowY = window.getPositionY();
            settings.fullscreen = PlatformRuntime.graphics.isFullscreen();
